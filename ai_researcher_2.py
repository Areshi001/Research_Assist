...
model_base = ChatGoogleGenerativeAI(model="gemini-2.5-flash", api_key=os.getenv("GOOGLE_API_KEY"))
model = model_base.bind_tools(tools)

# Step4: Setup StateGraph (Advanced / Complex mode)
...
# Compile the advanced graph (thread_id is passed per-session from frontend)
advanced_graph = workflow.compile(checkpointer=checkpointer)

# Step5: Simple graph (Beginner mode) via create_react_agent
simple_graph = create_react_agent(model_base, tools=tools)

# Step6: Shared system prompt
...
render it as a LaTeX PDF. Make sure the TEX file is correct with no errors so that the PDF exports cleanly. When you give papers references, always attach the pdf links to the paper.

CRITICAL FORMATTING INSTRUCTIONS:
Always format your chat responses beautifully using Markdown. Use **bolding**, *italics*, bulleted lists, and `inline code` to make your responses easy to read. Structure your thoughts using ### headers where appropriate. Do not output raw JSON or internal blocks to the user."""